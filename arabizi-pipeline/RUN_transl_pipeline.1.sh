...
TEST=example.1/0-small-example-mad # Set this to the filename to transliterate (the input to the pipeline will be $TEST.arabizi)
BASEDIR=.
...
if [ 1 == 1 ]; then
    echo "# tokenize and lowercase the Arabizi file => $TEST.arabizi.ENTOK.lc" 
    cat $TEST.arabizi | perl $TOKENIZE_EN - - | perl $LOWERCASE > $TEST.arabizi.ENTOK.lc

    echo "# extract dictionary of Arabizi test => $TEST.arabizi.ENTOK.lc.dict"
    cat $TEST.arabizi.ENTOK.lc | perl $TRAINDICT  > $TEST.arabizi.ENTOK.lc.dict

    echo "# get possible Arabizi->Arabic mappings => $TEST.lc.az-ar.lex"
